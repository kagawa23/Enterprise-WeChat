...
const conf =require("../config/sqlconfig");
...
var selectJoin =function(callBack){
     getConnection(function(connection){
        var ps = new mssql.PreparedStatement(connection);
        //SELECT * FROM sys_user u JOIN sys_userRoleMap r ON
        //u.UserCode =r.UserCode WHERE RoleCode ='XiaoShou'
        var sql ="SELECT u.*,r.RoleCode FROM sys_user u JOIN sys_userRoleMap r ON u.UserCode =r.UserCode WHERE RoleCode ='XiaoShou'";
        ps.prepare(sql, function (err) {
            if (err)
                console.log(err);
            ps.execute("", function (err, recordset) {
